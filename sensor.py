"""Sunways solar inverter interface."""
import asyncio
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    """Set up the Sunways sensors."""

    remove_interval_update = None
    
    # Init all sensors
    sensor_def = pysunways.Sensors()
    ...
    try:
        sunways = pysunways.Sunways(config[CONF_HOST], **kwargs)
        done = await sunways.read(sensor_def)
    except  pysunways.UnauthorizedException:
        _LOGGER.error("Username and/or password is wrong")
        return
    except  pysunways.UnexpectedResponseException as err:
        _LOGGER.error(
            "Error in Sunways, please check host/ip address. Original error: %s", err
        )
        return
    ...
    for sensor in sensor_def:
        hass_sensors.append(
            SUNWAYSsensor(sunways.serialnumber, sensor, inverter_name=config.get(CONF_NAME))
        )

    async_add_entities(hass_sensors)

    async def async_sunways():
        """Update all the Sunways sensors."""
        tasks = []

        values = await sunways.read(sensor_def)

        for sensor in hass_sensors:
            state_unknown = False
            if not values:
                # Sunways inverters are powered by DC via solar panels and thus are
...
class SUNWAYSsensor(Entity):
    """Representation of a Sunways sensor."""

    def __init__(self, serialnumber, pysaj_sensor, inverter_name=None):
        """Initialize the sunways sensor."""
        self._sensor = pysunways_sensor
    ...
    def name(self):
        ...
        if self._inverter_name:
            return f"sunways_{self._inverter_name}_{self._sensor.name}"

        return f"sunways_{self._sensor.name}"
    ...
    def unit_of_measurement(self):
        ...
        return SUNWAYS_UNIT_MAPPINGS[self._sensor.unit]
    ...
    def should_poll(self) -> bool:
        """Sunways sensors are updated & don't poll."""
        return False
